...
%An example of non functional requirements. In test cases where we we want an
%average we add an additional cell in bold
...
browser when its url is entered and that pictures can be uploaded and viewed on
the site.
...
the broswer does not crash.  Make sure there are poses shown on the site.
...
\end{flushleft}
 ...
 \bf Test & \bf User & \bf Time & \bf Result \\
 \hline\hline
   Search for "Woman" & Nick & 3 seconds  & Pass\\ %fill in these entries once we have them
 \hline\hline
