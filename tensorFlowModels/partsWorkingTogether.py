...
def enqueue(sess):
  ...
  while under < (max -20):
    ...
    if upper <= max:
        ...
        for x in filepaths:
            file_content = tf.read_file('/home/david/FLIC-full/images/' + 'rescale_'+row[0].strip())
            image_data = tf.image.decode_jpeg(file_content, channels=NUM_CHANNELS)
            image_data_list.append(train_image.set_shape([IMAGE_HEIGHT, IMAGE_WIDTH, NUM_CHANNELS]))
            for y in range(0,num_parts):
                curr_target[x, y, :] = np.array(encode_gauss_map(1,1))
